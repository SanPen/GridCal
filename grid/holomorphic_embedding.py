...
from numpy import where, zeros, ones, mod, conj, array, dot, complex64, complex128 #, complex256
...
def calc_X(n, k, nbus, R, C, Yred):
    ...
    result = conj(Yred[k, :].dot(C[n, :]) - Yred[k, k] * C[n, k])[0]
...
def helm(admittances, slackIndices, maxcoefficientCount, powerInjections, voltageSetPoints, types, eps=1e-3):
    ...
    last_valid_voltages = zeros(nbus)
    solve =factorized(Yred)
    correct = True
    error = 0
    while n <= maxcoefficientCount and not converged:
        ...
        if valid:
            last_valid_voltages = voltages_vector
        ...
        error = abs(calc_error(admittances, voltages_vector, S))
    ...
    print('\nv=')
    print(abs(last_valid_voltages))
    print('Error', max(error))
    print('converged', converged)
    print('Done.')

    return last_valid_voltages  #, C, W, X, R, H, Yred, Yrow, Iinj, errors