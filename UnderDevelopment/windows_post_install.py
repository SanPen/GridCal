# postinstall script for GridCal
"""Create GridCal start menu and desktop entries"""
import os
import sys
...
EWS = "Edit with GridCal"
KEY_C = r"Software\Classes\%s"
KEY_C0 = KEY_C % r"Python.%sFile\shell\%s"
KEY_C1 = KEY_C0 + r"\command"


# ability to run GridCal-win-post-install outside of bdist_wininst installer
...
def install():
    ...
    # Create GridCal start menu folder
    ...
    start_menu = osp.join(get_special_folder_path('CSIDL_PROGRAMS'),
                          'GridCal (Py%i.%i %i bit)' % (sys.version_info[0],
                                                       sys.version_info[1],
                                                       struct.calcsize('P')*8))
    ...
    fname = osp.join(start_menu, 'GridCal (full).lnk')
    create_shortcut(python, desc, fname, '"%s"' % script, workdir,
                    osp.join(ico_dir, 'GridCal.ico'))
    file_created(fname)

    fname = osp.join(start_menu, 'GridCal-Reset all settings.lnk')
    create_shortcut(python, 'Reset GridCal settings to defaults',
    ...
    winreg.SetValueEx(winreg.CreateKey(root, KEY_C1 % ("", EWS)),
                      "", 0, winreg.REG_SZ,
                      '"%s" "%s\Scripts\GridCal" "%%1"' % (pythonw, sys.prefix))
    ...
    fname = osp.join(desktop_folder, 'GridCal.lnk')
    desc = 'Scientific Python Development EnvironmEnt, an alternative to IDLE'
    ...
    for key in (KEY_C1 % ("", EWS), KEY_C1 % ("NoCon", EWS),
        ...
        try:
            ...
            if not is_bdist_wininst:
                print("Successfully removed GridCal shortcuts from Windows "\
    ...
    if not is_bdist_wininst:
        ...
        if osp.isfile(fname):
            try:
                ...
                print("Successfully removed GridCal shortcuts from your desktop.",
                      file=sys.stdout)
        # clean up startmenu
        start_menu = osp.join(get_special_folder_path('CSIDL_PROGRAMS'),
                              'GridCal (Py%i.%i %i bit)' % (sys.version_info[0],
        ...
        if osp.isdir(start_menu):
            for fname in os.listdir(start_menu):
                try:
                    ...
                    print("Successfully removed GridCal shortcuts from your "\
            ...
            try:
                ...
                print("Successfully removed GridCal shortcut folder from your "\
                      " start menu.", file=sys.stdout)
