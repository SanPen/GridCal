...
v_sin = Var('v_sin')
delta_dt = DiffVar.get_or_create('delta_dt', base_var = delta_1)
dg_dt = DiffVar.get_or_create('dg_dt', base_var = d_G1)
...
u = Var('u')
domega_dt = DiffVar.get_or_create('domega_dt', base_var=omega_1)
det_dt = DiffVar.get_or_create('det_dt', base_var=et_1)
...
generator_block_1 = DiffBlock(
    ...
    algebraic_vars=[psid_1, psiq_1, i_d_1, i_q_1, v_d_1, v_q_1, te_1, P_G1, Q_G1, tm_1, u_cos, u_sin], 
    parameters=[],
...
"""generator_block_1 = DiffBlock(
    ...
    algebraic_eqs=[
        ...
        Q_G1 - (v_q_1 * i_d_1 - v_d_1 * i_q_1),
        tm_1 - (tm0_1 + Kp_1 * (omega_1 - omega_ref_1) + Ki_1 * et_1),
        int_sin2 - 0.5*((delta_1-d_G1)-u_sin*u_cos), 
        int_sin2cos2 - 1/32*(4*(delta_1-d_G1) - (4*u_cos**3*u_sin - 4*u_sin**3*u_cos)),
        dt_int_sin2  - (delta_dt - dg_dt)*u_sin**2,
        dt_int_sin2cos2 - (delta_dt - dg_dt)*u_sin**2*u_cos**2
    ],
    algebraic_vars=[psid_1, psiq_1, i_d_1, i_q_1, v_d_1, v_q_1, te_1, P_G1, Q_G1, tm_1, u_sin, u_cos, int_sin2, int_sin2cos2], 
    parameters=[],
    diff_vars = [delta_dt, dg_dt, dt_int_sin2, dt_int_sin2cos2]
)"""
...
d_G10 = np.angle(v1),
delta_1_0 = delta0_1
u_sin0 =np.sin(delta_1_0 - d_G10)
u_cos0 =np.cos(delta_1_0 - d_G10)

vars_mapping = {
    u_sin : u_sin0,
    u_cos : u_cos0,
    #int_sin2: 0.5*((delta_1_0-d_G10)-u_sin0*u_cos0),
    #int_sin2cos2: 1/32*(4*(delta_1_0-d_G10) - (4*u_cos0**3*u_sin0 - 4*u_sin0**3*u_cos0)),
...
dx0 = slv.build_init_diffvars_vector(diff_mapping)
...
if not isinstance(slv, DiffBlockSolver):
    t, y = slv.simulate(
        t0=0,
        t_end=5.0,
        h=0.001,
...
plt.plot(t, y[:, slv.get_var_idx(u_cos)], label="usin (pu)", color='blue')
plt.plot(t, y[:, slv.get_var_idx(u_sin)], label="ucos (pu)", color='yellow')
