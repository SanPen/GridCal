...
# Buses
bus1 = gce.Bus(name="Bus1", Vnom=230, is_slack=True)
...
# Lines
# S = I*V*sqrt(3)
line1 = grid.add_line(gce.Line(name="line 1-2", bus_from=bus1, bus_to=bus2, r = 0.005, x=0.05, b=0.075, rate=100.0))


# loads
l1 = grid.add_load(bus=bus2, api_obj=gce.Load(P= 11.59, Q= -0.735))

# Generators
gen1 = gce.Generator(name="Gen1", P=7.45861, vset=1.0, Snom = 900)
...
generator_block = Block(
    ...
    algebraic_eqs=[
        ...
        P_g - (v_d * i_d + v_q * i_q),
        Q_g - (v_q * i_d - v_d * i_q),
        # tm_ref - (tm0 + Kp * (omega - omega_ref) + Ki * et)
    ],
    algebraic_vars=[psid, psiq, i_d, i_q, v_d, v_q, t_e, P_g, Q_g],  #tm_ref
    parameters=[]
)
...
event1 = RmsEvent('Load', Pl0, 2500, Sb2.real - 0.5)
