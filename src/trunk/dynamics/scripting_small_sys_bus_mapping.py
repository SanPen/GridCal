...
line_block = Block(
    # algebraic_eqs=[
    #     Pline_from - ((Vm1 ** 2 * g) - g * Vm1 * Vm2 * cos(
    #         Va1 - Va2) + b * Vm1 * Vm2 * cos(Va1 - Va2 + np.pi / 2)),
    #     Qline_from - (Vm1 ** 2 * (-bsh / 2 - b) - g * Vm1 * Vm2 * sin(
    #         Va1 - Va2) + b * Vm1 * Vm2 * sin(Va1 - Va2 + np.pi / 2)),
    #     Pline_to - ((Vm2 ** 2 * g) - g * Vm2 * Vm1 * cos(
    #         Va2 - Va1) + b * Vm2 * Vm1 * cos(Va2 - Va1 + np.pi / 2)),
    #     Qline_to - (Vm2 ** 2 * (-bsh / 2 - b) - g * Vm2 * Vm1 * sin(
    #         Va2 - Va1) + b * Vm2 * Vm1 * sin(Va2 - Va1 + np.pi / 2)),
    # ],
    algebraic_vars=[Pline_from, Pline_to, Qline_from, Qline_to],
    parameters=[],
    external_mapping = {
        DynamicVarType.Pf: Pline_from,
        DynamicVarType.Pt: Pline_to,
        DynamicVarType.Qf: Qline_from,
        DynamicVarType.Qt: Qline_to,
    }
)
Vmf = line.bus_from.rms_model.model.V("Vm")
line_block.algebraic_eqs.append(line_block.E(DynamicVarType.Pf) - ((Vmf ** 2 * g) - g * Vm1 * Vm2 * cos(Va1 - Va2) + b * Vm1 * Vm2 * cos(Va1 - Va2 + np.pi / 2)))
# ----------------------------------------------------------------------------------------------------------------------
# Load
# ----------------------------------------------------------------------------------------------------------------------
