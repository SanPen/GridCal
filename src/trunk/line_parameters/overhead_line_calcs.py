...
Functions:
    ...
    calc_z_ii         Calculates self impedance term (in Ohm/km)
    calc_z_ij       Calculates mutual impedance term (in Ohm/km)
...
def calc_L_int(type, r, q):
    ...
    if type == 'solid':
        ...
        L_int = mu_0 / 2 / np.pi * (q ** 4 / (r ** 2 - q ** 2) ** 2 * np.log(r / q) - (3 * q ** 2 - r ** 2) / (
                    4 * (r ** 2 - q ** 2)))
...
def calc_GMR(type, r, q):
    ...
    if type == 'solid':
        ...
        GMR = r * np.exp((3 * q ** 2 - r ** 2) / (4 * (r ** 2 - q ** 2)) - q ** 4 / (r ** 2 - q ** 2) ** 2 * np.log(
            r / q))  # Tubular conductor GMR [m]
...
def calc_self_Z(R_int, cond_type, r, q, h_i, f, rho, err_tol=1e-6):
    ...
    Usage:
        self_Z = calc_z_ii(R_int, cond_type, r, q, h_i, f, rho, err_tol=1e-6)
...
def calc_mutual_Z(cond_type, r, q, h_i, h_k, x_ik, f, rho, err_tol=1e-6):
    ...
    Usage:
        mutual_Z = calc_z_ij(cond_type, r, q, h_i, h_k, x_ik, f, rho, err_tol=1e-6)
...
def calc_Z_matrix(line_dict):
    ...
    if mode == 'carson':
        ...
        for i in range(n_c):
            for j in range(n_c):
                if i == j:
                    Z[i, j] = calc_Dubanton_Z('self', cond_R[i], cond_type[i], cond_r[i], cond_q[i], cond_h[i],
                                              cond_h[j], cond_x[i] - cond_x[j], f, rho)
                else:
                    Z[i, j] = calc_Dubanton_Z('mutual', cond_R[i], cond_type[i], cond_r[i], cond_q[i], cond_h[i],
                                              cond_h[j], cond_x[i] - cond_x[j], f, rho)
    ...
    for i in range(n_c):
        for j in range(n_c):
            if i == j:
                ...
                d = np.sqrt(
                    (cond_h[i] - cond_h[j]) ** 2 + (
                                cond_x[i] - cond_x[j]) ** 2)  # Distance between conductors i and k [m]
...
if __name__ == "__main__":
    ...
    line_dict = {
        ...
        'phase_x': [Xa - bundle / 2, Xa + bundle / 2, Xb - bundle / 2, Xb + bundle / 2, Xc - bundle / 2, Xc + bundle / 2],
        # Phase conductor x-axis coordinates [m]
    ...
    # Impedance matrix
    Zprimitive, n_p, n_e = calc_Z_matrix(line_dict)
    Zreduced, phases_vector_ = wire_bundling([1, 2, 3], Zprimitive, [1, 1, 2, 2, 3, 3])
    ...
    # Admittance matrix
    Yprimitive, n_p, n_e = calc_Y_matrix(line_dict)
    Yreduced, phases_vector_ = wire_bundling_shunt([1, 2, 3], Yprimitive, [1, 1, 2, 2, 3, 3])
