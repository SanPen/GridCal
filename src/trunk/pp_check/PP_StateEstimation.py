...
LP1 = net1.res_line.loc[0, 'p_from_mw'] + \
      abs(np.random.randn(1) * noise_percentage * np.exp(0.1j * np.random.randn(1)))[0]
LQ1 = net1.res_line.loc[0, 'q_from_mvar'] + \
      abs(np.random.randn(1) * noise_percentage * np.exp(0.1j * np.random.randn(1)))[0]
LP2 = net1.res_line.loc[1, 'p_from_mw'] + \
      abs(np.random.randn(1) * noise_percentage * np.exp(0.1j * np.random.randn(1)))[0]
LQ2 = net1.res_line.loc[1, 'q_from_mvar'] + \
      abs(np.random.randn(1) * noise_percentage * np.exp(0.1j * np.random.randn(1)))[0]
# now the network
