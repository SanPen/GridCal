...
def generate_uniform_binary_pop(problem_ptp, pop_size):
    """
    Based on the Platypus library
    ...
    :param problem_ptp: problem object from platypus library
    :param pop_size: number of individuals in the population
    :return: initial_population: list of platypus solution objects
    """
    # inputs:
    n_samples = pop_size
    ...
    predefined_population = np.array(ones_into_array_bin)
...
def NSGA_3_platypus(obj_func,
    ...
    init_pop_obj = generate_uniform_binary_pop(problem_ptp, pop_size)  # initialising population matrix
    ...
    algorithm = NSGAIII(
        ...
        generator=InjectedPopulation(init_pop_obj),
    ...
    for it in range(iterations):
        ...
        for s in range(len(algorithm.result)):
            ...
            # all_variables_decoded = []
            variables_decoded2 = [problem_ptp.types[i].decode(all_variables_to_decode[i]) for i in
                                  range(len(algorithm.result[s].variables))]
    ...
    return all_variables_decoded, res_objective_all
