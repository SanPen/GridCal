...
class AdequacyResults(ResultsTemplate):
    ...
    def __init__(self, investment_groups_names: StrVec, max_eval: int):
        ...
        self._lole: Vec = np.zeros(max_eval, dtype=float)
        ...
        self.register(name='_lole', tpe=Vec)
    ...
    def calculate_tech_score_magnitudes(self):
        ...
        self.losses_majority_magnitude = get_max_magnitude(self.lole_mag)
    ...
    def add(self,
        ...
        # self.lole_mag.append(self.calculate_magnitude(lole))
    ...
    def trim(self):
        ...
        if len(self._capex) > self.__eval_index:
            ...
            self._lole = self._lole[:self.__eval_index]
    ...
    def mdl(self, result_type) -> "ResultsTable":
        ...
        if result_type in (ResultTypes.InvestmentsReportResults, ResultTypes.InvestmentsParetoReportResults):
            ...
            data = np.c_[
                ...
                self._lole + self._voltage_score + self._overload_score,
                self._financial,
                self._f_obj,
                used_investments
            ]
            ...
            columns = ["Investment cost (M€)", "Technical cost (M€)", "LOLE (MWh)", "Overload cost (M€)",
                       "Voltage cost (M€)"]
            data = np.c_[
                self._financial, self._lole * self.losses_scale + self._voltage_score * self.voltage_scale + self._overload_score, self._lole, self._overload_score, self._voltage_score]
            y_label = ''
            title = ''
            ...
            technical_score = self._lole * self.losses_scale + self._voltage_score * self.voltage_scale + self._overload_score
            ...
            sc2 = ax3[0, 1].scatter(scaled_financial_score, self._lole,
                                    c=np.divide(self._financial, self.losses_scale) + self._lole)
