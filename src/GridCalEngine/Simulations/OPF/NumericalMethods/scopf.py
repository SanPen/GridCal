...
def compute_analytic_structures(x, mu, lmbda, compute_jac: bool, compute_hess: bool, admittances, Cg, R, X, Sd, slack,
    ...
    :param nshed: Number of load shedding buses
    ...
    :param W_k_vec: Vector of objective functions from the SPs
    :param Z_k_vec: Vector of slopes from the SPs
    :param u_j_vec: Vector of unknowns where the slopes for the SPs are applied
    :param acopf_mode: Mode of the AC OPF
    ...
    fx, Gx, Hx, fxx, Gxx, Hxx, der_times = jacobians_and_hessians_scopf(x=x, c1=c1, c2=c2, c_s=c_s, c_v=c_v, Cg=Cg,
                                                                        Cf=Cf,
                                                                        Ct=Ct, Inom=Inom, Yf=Yf,
                                                                        Yt=Yt, Ybus=Ybus, Sbase=Sbase, mon_br_idx=il,
                                                                        ig=ig,
                                                                        nshed=nshed,
                                                                        ...
                                                                        alltapt=alltapt, F_hvdc=fdc, T_hvdc=tdc,
                                                                        nsh=nsh,
                                                                        k_m=k_m, k_tau=k_tau, mu=mu, lmbda=lmbda,
                                                                        R=R, X=X,
                                                                        F=from_idx, T=to_idx, ctQ=ctQ,
                                                                        Z_k_vec=Z_k_vec,
                                                                        acopf_mode=acopf_mode,
...
@dataclass
class NonlinearSCOPFResults:
    """
    Numerical non linear SCOPF results
    """
    Va: Vec = None
    Vm: Vec = None
    S: CxVec = None
    ...
    def initialize(self, nbus: int, nbr: int, nsh: int, nshed: int, ng: int, nhvdc: int, ncap: int):
        """
...
def scopf_subproblem(nc: NumericalCircuit,
    ...
    # PV buses are identified by those with the same upper and lower limits for the voltage
    ...
    if len(id_Vm_max0) != 0:
        for i in id_Vm_max0:
            logger.add_warning('Upper voltage limits are set lower to the lower limit. Correcting to 1.1 p.u.',
                               device="Bus " + str(i))
    ...
    non_slack_idx = np.r_[np.arange(0, 2 * nbus + 2 * n_gen_disp),
    ...
    capacity_nodes_idx = np.array([])
    nodal_capacity_sign = 1
    ...
    if debug:
        ...
        if use_autodiff:
            ...
            result = interior_point_solver(x0=x0, n_x=NV, n_eq=NE, n_ineq=NI,
                                           ...
                                           arg=(admittances, Cg, Sd, slack, from_idx, nshed, to_idx, pq, pv,
            ...
            result, times = interior_point_solver(x0=x0, n_x=NV, n_eq=NE, n_ineq=NI,
                                                  ...
                                                  arg=(admittances, Cg, R, X, Sd, slack, from_idx, to_idx, f_nd_hvdc,
                                                       ...
                                                       opf_options.ips_control_q_limits, None, None, None,
    ...
    if not result.converged or result.converged:
        ...
        for i in range(nbus):
            if abs(result.dlam[i]) >= 1e-3:
                logger.add_warning('Nodal P Balance convergence tolerance not achieved',
                                   device_property="dlam",
                                   device=str(i),
                                   value=str(result.dlam[i]),
                                   expected_value='< 1e-3')

            if abs(result.dlam[nbus + i]) >= 1e-3:
                logger.add_warning('Nodal Q Balance convergence tolerance not achieved',
                                   device_property="dlam",
                                   device=str(i),
...
def scopf_MP_OPF(nc: NumericalCircuit,
                 ...
                 u_j_vec: Union[List[Vec], None] = None) -> NonlinearSCOPFResults:
    """
    ...
    if opf_options.ips_control_q_limits:
        NI = 2 * n_br_mon + 2 * npq + ngen + 4 * n_gen_disp + 2 * ntapm + 2 * ntapt + 2 * n_disp_hvdc + nsl + 2 * nshed + n_cuts
    else:
        # No Reactive constraint (power curve)
        NI = 2 * n_br_mon + 2 * npq + 4 * n_gen_disp + 2 * ntapm + 2 * ntapt + 2 * n_disp_hvdc + nsl + 2 * nshed + n_cuts
        ...
        if use_autodiff:
            ...
            result, times = interior_point_solver(x0=x0, n_x=NV, n_eq=NE, n_ineq=NI,
                                                  ...
                                                  arg=(admittances, Cg, R, X, Sd, slack, from_idx, to_idx, f_nd_hvdc,
                                                       ...
                                                       alltapm, alltapt, k_m, k_tau, c0, c1, c2, c_s, c_v, Sbase, rates,
                                                       br_mon_idx, n_br_mon, gen_disp_idx, gen_nondisp_idx, Sg_undis,
                                                       opf_options.ips_control_q_limits, W_k_vec, Z_k_vec, u_j_vec,
                                                       opf_options.acopf_mode),
                                                  verbose=opf_options.verbose,
                                                  max_iter=opf_options.ips_iterations,
                                                  tol=opf_options.ips_tolerance,
...
def run_nonlinear_MP_opf(nc: NumericalCircuit,
                         ...
                         logger: Logger = Logger()) -> NonlinearSCOPFResults:
    """
    Run optimal power flow for a NumericalCircuit
    :param nc: NumericalCircuit
    :param opf_options: OptimalPowerFlowOptions
    :param pf_options: PowerFlowOptions
    ...
    :param optimize_nodal_capacity:
    :param load_shedding: Load shedding?
    :param nodal_capacity_sign:
    :param capacity_nodes_idx:
    ...
    for i, island in enumerate(islands):
        ...
        island_res = scopf_MP_OPF(nc=island,
