...
def jacobians_and_hessians(x, c1, c2, Cg, Cf, Ct, Yf, Yt, Ybus, Sbase, il, ig, nig, slack, no_slack, pq, pv, alltapm,
    ...
    if compute_jac:
        ...
        #
        # HSf = sp.hstack([dAf_dVa, dAf_dVm, lil_matrix((nl, 2 * Ng))])
        # HSt = sp.hstack([dAt_dVa, dAt_dVm, lil_matrix((nl, 2 * Ng))])
        ...
        Hvau = sp.hstack([Hvau_, lil_matrix((N - len(slack), N + 2 * Ng))])
        Hval = sp.hstack([Hval_, lil_matrix((N - len(slack), N + 2 * Ng))])
        ...
        # Hx = sp.vstack([HSf, HSt, Hvu, Hvl, Hpu, Hpl, Hqu, Hql]).T.tocsc()
        Hx = sp.vstack([HSf, HSt, Hvu, Hpu, Hqu, Hvl, Hpl, Hql]).T.tocsc()
        # Hx = sp.vstack([HSf, HSt, Hvu, Hvl, Hvau, Hval, Hpu, Hpl, Hqu, Hql]).T.tocsc()
    else:
        fx = None
        Gx = None
        Hx = None
    ...
    if compute_hess:
        ...
        # Add all
        # G1 = sp.hstack([Gvv_p.real + Gvv_q.imag, Gva_p.real + Gva_q.imag, lil_matrix((N, 2 * Ng))])
        # G2 = sp.hstack([Gav_p.real + Gav_q.imag, Gaa_p.real + Gaa_q.imag, lil_matrix((N, 2 * Ng))])

        G1 = sp.hstack([Gaa_p.real + Gaa_q.imag, Gav_p.real + Gav_q.imag, lil_matrix((N, 2 * Ng))])
        G2 = sp.hstack([Gva_p.real + Gva_q.imag, Gvv_p.real + Gvv_q.imag, lil_matrix((N, 2 * Ng))])
        ...
        # H1 = sp.hstack([Hfvmvm + Htvmvm, Hfvavm + Htvmva, lil_matrix((N, 2 * Ng))])
        # H2 = sp.hstack([Hfvavm + Htvavm, Hfvava + Htvava, lil_matrix((N, 2 * Ng))])
