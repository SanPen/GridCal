...
def eval_h_scopf(x: Vec, Yf: csc_matrix, Yt: csc_matrix, from_idx: Vec, to_idx: Vec, nslcap: int,
                 pq: Vec, k_m: Vec, k_tau: Vec, Cg: csc_matrix, Inom: Vec, nshed: int, Sd: Vec,
                 Vm_max: Vec, Vm_min: Vec, Pg_max: Vec, Pg_min: Vec, Qg_max: Vec, Qg_min: Vec, tapm_max: Vec,
                 tapm_min: Vec, tapt_max: Vec, tapt_min: Vec, Pdcmax: Vec, rates: Vec, il: Vec, ig: Vec,
                 ctQ: bool, W_k_vec: Vec, Z_k_vec: List[Vec], u_j_vec: List[Vec], acopf_mode: AcOpfMode) -> Tuple[
    Vec, CxVec, CxVec]:
    """
    Calculates the inequality constraints at the current state (given by x)
    :param x: State vector
    ...
    va, vm, Pg, Qg, Pshed, sl_sf, sl_st, sl_vmax, sl_vmin, slcap, tapm, tapt, Pfdc = x2var(x, nVa=N, nVm=N, nPg=Ng,
                                                                                           ...
                                                                                           nslcap=nslcap,
                                                                                           acopf_mode=acopf_mode)
    x_new = var2x(va, vm, Pg, Qg, Pshed, sl_sf, sl_st, sl_vmax, sl_vmin, slcap, tapm, tapt, Pfdc)
    ...
    if W_k_vec is not None:
        n_sps = len(W_k_vec)
        h_scopf = np.zeros(n_sps, dtype=np.float64)
        for k in range(n_sps):
            ...
            # selecting for x_new only Pg
            h_scopf[k] = W_k + Z_k @ (x_new[2 * N: 2 * N + Ng] - u_j)
    else:
        h_scopf = []
...
def jacobians_and_hessians(x: Vec, c1: Vec, c2: Vec, c_s: Vec, c_v: Vec, Cg: csc_matrix, Cf: csc, Ct: csc, Inom: Vec,
    ...
    :param ig: Index of dispatchable generators
    :param nshed: Number of load shedding buses
    :param shedding_cost: Cost of each load shedding
    :param slack: Index of slack buses
    :param nslcap:
    :param nodal_capacity_sign:
    :param capacity_nodes_idx:
...
def jacobians_and_hessians_scopf(x: Vec, c1: Vec, c2: Vec, c_s: Vec, c_v: Vec, Cg: csc_matrix, Cf: csc, Ct: csc,
                                 Inom: Vec,
                                 Yf: csc_matrix, Yt: csc_matrix, Ybus: csc_matrix, Sbase: float, mon_br_idx: IntVec,
                                 ig: IntVec, nshed: int, shedding_cost: Vec,
                                 slack: Vec, nslcap: int, nodal_capacity_sign: float, capacity_nodes_idx: IntVec,
                                 pq: IntVec,
                                 pv: IntVec, alltapm: Vec, alltapt: Vec, F_hvdc: IntVec, T_hvdc: IntVec, nsh: int,
                                 k_m: IntVec, k_tau: IntVec, mu, lmbda, R: Vec, X: Vec, F: IntVec, T: IntVec,
                                 ctQ: bool, Z_k_vec: List[Vec], acopf_mode: AcOpfMode, compute_jac: bool,
                                 compute_hess: bool) -> Tuple[Vec, csc, csc, csc, csc, csc, Vec]:
    """
    Calculates the jacobians and hessians of the objective function and the equality and inequality constraints
    at the current state given by x
    ...
    :param T: Index of the 'to' bus for each line
    :param ctQ: Boolean that indicates if the Reactive control applies
    :param Z_k_vec: List of vectors of Z_k for each SCOPF scenario
    :param acopf_mode: AcOpfMode
    :param compute_jac: Boolean that indicates if the Jacobians have to be calculated
    :param compute_hess: Boolean that indicates if the Hessians have to be calculated
    :return: Jacobians and hessians matrices for the objective function and the equality and inequality constraints
    ...
    if compute_jac:
        ...
        if n_scopf > 0:
            # Convert Z_k_vec from list of vectors to matrix
            Z_k_mat = np.array(Z_k_vec)  # Maybe works?
        ...
        if n_scopf > 0:
            ...
            Hscopf = sp.hstack([lil_matrix((n_scopf, 2 * N)),
                                Z_k_mat,
                                lil_matrix((n_scopf, NV - (2 * N + Ng)))])
        ...
        Hx = sp.vstack([HSf, HSt, Hvu, Hpu, Hqu, Hpshedu, Hvl, Hpl, Hql, Hpshedl, Hslsf, Hslst, Hslvmax,
                        Hslvmin, Htapmu, Htapml, Htaptu, Htaptl, Hscopf, Hqmax, Hdcu, Hdcl])
