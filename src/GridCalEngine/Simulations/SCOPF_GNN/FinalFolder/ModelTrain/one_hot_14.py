...
# Load dataset
data = pd.read_csv(
    "/Users/CristinaFray/PycharmProjects/GridCal/src/GridCalEngine/Simulations/SCOPF_GNN/FinalFolder/ModelTrain/scopf_dataset_14_200.csv")
...
# Optimized model architecture from latest trial
class Net(nn.Module):
    def __init__(self, input_dim, output_dim):
        super(Net, self).__init__()
        self.model = nn.Sequential(
            nn.Linear(input_dim, 224),  # hidden1
            nn.ReLU(),
            nn.Dropout(0.26591002025583393),  # dropout
            nn.Linear(224, 63),  # hidden2
            nn.ReLU(),
            nn.Linear(63, output_dim)
        )
...
optimizer = optim.Adam(model.parameters(), lr=0.006689882051224795)
...
for epoch in range(EPOCHS):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), "../../ModelTraining/best_model_14_200.pt")
        patience_counter = 0
    else:
        patience_counter += 1
...
# Evaluation
model.load_state_dict(torch.load("../../ModelTraining/best_model_14_200.pt"))
model.eval()
