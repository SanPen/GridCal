...
from GridCalEngine.IO.cim.cgmes.cgmes_enums import cgmesProfile, DCPolarityKind, ControlAreaTypeKind, CsOperatingModeKind, AsynchronousMachineKind, CsPpccControlKind, DCConverterOperatingModeKind, BatteryStateKind, CurveStyle, UnitSymbol
...
class Equipment(PowerSystemResource):
	def __init__(self, rdfid='', tpe='Equipment'):
		...
		self.register_property(
			...
			description='''The aggregate flag provides an alternative way of representing an aggregated (equivalent) element. It is applicable in cases when the dedicated classes for equivalent equipment do not have all of the attributes necessary to represent the required level of detail.  In case the flag is set to ìtrueî the single instance of equipment represents multiple pieces of equipment that have been modelled together as an aggregate equivalent obtained by a network reduction procedure. Examples would be power transformers or synchronous machines operating in parallel modelled as a single aggregate power transformer or aggregate synchronous machine.  
