...
from GridCalEngine.IO.cim.cgmes.cgmes_enums import cgmesProfile, SynchronousMachineKind, GeneratorControlSource, WindingConnection, VsQpccControlKind, ControlAreaTypeKind, SVCControlMode, BatteryStateKind, DCPolarityKind, FuelType, AsynchronousMachineKind, PhaseCode, Currency, HydroPlantStorageKind, HydroTurbineKind, CurveStyle, OperationalLimitDirectionKind, LimitKind, UnitSymbol, CsPpccControlKind, CsOperatingModeKind, HydroEnergyConversionKind, SynchronousMachineOperatingMode, DCConverterOperatingModeKind, UnitMultiplier, VsPpccControlKind, RegulatingControlModeKind
...
class VsConverter(ACDCConverter):
	def __init__(self, rdfid='', tpe='VsConverter'):
		...
		self.register_property(
			...
			multiplier=UnitMultiplier.none,
			unit=UnitSymbol.none,
			description='''The maximum quotient between the AC converter voltage (Uc) and DC voltage (Ud). A factor typically less than 1. It is converterís configuration data used in power flow.''',
			profiles=[]
		)
		self.register_property(
			name='delta',
