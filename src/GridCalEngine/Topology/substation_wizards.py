...
def simple_bar(name, grid: MultiCircuit, n_lines: int, n_trafo: int, v_nom: float,
               lat:float| None = None, lon: float| None = None, country: Country = None, include_disconnectors: bool = True):
    """
    ...
    bar = dev.Bus(f"{name} bar", substation=substation, Vnom=220, voltage_level=vl,
                  width=max(n_lines, n_trafo)*bus_width + bus_width*2 + (dist*2-bus_width)*(max(n_lines, n_trafo)-1),
    ...
    if include_disconnectors:
        ...
        for i in range(n_lines):
            bus1 = dev.Bus(f"{name}_line_conn_{i}", substation=substation, Vnom=220, voltage_level=vl,
                           xpos=0 + i*dist*2, ypos=0, width=bus_width, country=country)
            bus2 = dev.Bus(f"LineBus2_{i}", substation=substation, Vnom=220, voltage_level=vl,
                           xpos=0 + i*dist*2, ypos=dist, width=bus_width, country=country)
            bus3 = dev.Bus(f"LineBus3_{i}", substation=substation, Vnom=220, voltage_level=vl,
        ...
        for i in range(n_trafo):
            bus1 = dev.Bus(f"{name}_trafo_conn_{i}", substation=substation, Vnom=220, voltage_level=vl,
                           xpos=0 + i*dist*2, ypos=dist*5, width=bus_width, country=country)
            bus2 = dev.Bus(f"trafo2_{i}", substation=substation, Vnom=220, voltage_level=vl,
                           xpos=0 + i*dist*2, ypos=dist*4, width=bus_width, country=country)
        ...
        for i in range(n_lines):
            bus1 = dev.Bus(f"{name}_line_conn_{i}", substation=substation, voltage_level=vl)
        ...
        for i in range(n_trafo):
            bus1 = dev.Bus(f"{name}_trafo_conn_{i}", substation=substation, voltage_level=vl)
...
def simple_bar_with_bypass(name, grid: MultiCircuit, n_lines: int, n_trafo: int):
    """
