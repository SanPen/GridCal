...
class LpModel:
    ...
    def status2string(self, stat: int) -> str:
        ...
        :return:
        """
        if stat == ort.Solver.OPTIMAL:
            return "optimal"
        if stat == ort.Solver.FEASIBLE:
            return "feasible"
        if stat == ort.Solver.INFEASIBLE:
            return "infeasible"
        if stat == ort.Solver.UNBOUNDED:
            return "unbounded"
        if stat == ort.Solver.ABNORMAL:
            return "abnormal"
        if stat == ort.Solver.MODEL_INVALID:
            return "model invalid"
        if stat == ort.Solver.NOT_SOLVED:
            return "not solved"
        return 'unknown'
