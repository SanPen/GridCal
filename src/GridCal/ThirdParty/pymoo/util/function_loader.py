...
from GridCal.ThirdParty.pymoo.config import Config


def get_functions():
    
    from GridCal.ThirdParty.pymoo.util.nds.fast_non_dominated_sort import fast_non_dominated_sort
    from GridCal.ThirdParty.pymoo.util.nds.efficient_non_dominated_sort import efficient_non_dominated_sort
    from GridCal.ThirdParty.pymoo.util.nds.tree_based_non_dominated_sort import tree_based_non_dominated_sort
    from GridCal.ThirdParty.pymoo.decomposition.util import calc_distance_to_weights
    from GridCal.ThirdParty.pymoo.util.misc import calc_perpendicular_distance
    from GridCal.ThirdParty.pymoo.util.hv import hv
    from GridCal.ThirdParty.pymoo.util.stochastic_ranking import stochastic_ranking
    from GridCal.ThirdParty.pymoo.util.mnn import calc_mnn, calc_2nn
    from GridCal.ThirdParty.pymoo.util.pruning_cd import calc_pcd
...
class FunctionLoader:
    ...
    def __init__(self) -> None:
        ...
        if Config.warnings["not_compiled"] and not self.is_compiled:
            ...
            print("from GridCal.ThirdParty.pymoo.config import Config")
...
def is_compiled():
    try:
        from GridCal.ThirdParty.pymoo.cython.info import info
        if info() == "yes":
            return True
        else:
            return False
