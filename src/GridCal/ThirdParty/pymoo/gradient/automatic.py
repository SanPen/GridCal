...
from GridCal.ThirdParty.pymoo.core.meta import Meta
from GridCal.ThirdParty.pymoo.core.problem import Problem, ElementwiseEvaluationFunction
...
class ElementwiseEvaluationFunctionWithGradient(ElementwiseEvaluationFunction):
    ...
    def __call__(self, x):
        ...
        if TOOLBOX == "jax.numpy":
            from GridCal.ThirdParty.pymoo.gradient.grad_jax import jax_elementwise_value_and_grad
            out, grad = jax_elementwise_value_and_grad(f, x)

        elif TOOLBOX == "autograd.numpy":
            from GridCal.ThirdParty.pymoo.gradient.grad_autograd import autograd_elementwise_value_and_grad
...
class AutomaticDifferentiation(Meta, Problem):
    ...
    def do(self, x, return_values_of, *args, **kwargs):
        ...
        if TOOLBOX == "jax.numpy":
            from GridCal.ThirdParty.pymoo.gradient.grad_jax import jax_vectorized_value_and_grad
            out, grad = jax_vectorized_value_and_grad(f, x)

        elif TOOLBOX == "autograd.numpy":
            from GridCal.ThirdParty.pymoo.gradient.grad_autograd import autograd_vectorized_value_and_grad
