...
def get_load_data(circuit: MultiCircuit, bus_dict, opf_results=None, time_series=False, opf=False, ntime=1):
    ...
    if opf:
        data = ds.LoadOpfData(nelm=len(devices), nbus=len(circuit.buses), ntime=ntime)
    else:
        data = ds.LoadData(nelm=len(devices), nbus=len(circuit.buses), ntime=ntime)
...
def get_shunt_data(circuit: MultiCircuit, bus_dict, Vbus, logger: Logger, time_series=False, ntime=1, use_stored_guess=False):
    ...
    data = ds.ShuntData(nelm=len(devices), nbus=len(circuit.buses), ntime=ntime)
...
def get_generator_data(circuit: MultiCircuit, bus_dict, Vbus, logger: Logger,
    ...
    if opf:
        ...
        data = ds.GeneratorData(nelm=len(devices), nbus=len(circuit.buses), ntime=ntime)
...
def get_battery_data(circuit: MultiCircuit, bus_dict, Vbus, logger: Logger,
    ...
    if opf:
        ...
        data = ds.BatteryData(nelm=len(devices), nbus=len(circuit.buses), ntime=ntime)
...
def get_branch_data(circuit: MultiCircuit, bus_dict, Vbus, apply_temperature,
    ...
    if opf:
        ...
        data = ds.BranchData(nelm=nbr, nbus=len(circuit.buses), ntime=ntime)
    ...
    for i, elm in enumerate(circuit.transformers2w):
        ...
        if time_series:
            if opf_results is not None:
                ...
                data.tap_module[ii] = elm.tap_module_prof
                data.tap_angle[ii, :] = elm.angle_prof
        else:
            if opf_results is not None:
                data.tap_module[ii] = elm.tap_module
                data.tap_angle[ii] = opf_results.phase_shift[ii]
...
def get_hvdc_data(circuit: MultiCircuit, bus_dict, bus_types, time_series=False, ntime=1,
    ...
    :return:
    """
    data = ds.HvdcData(nelm=len(circuit.hvdc_lines), nbus=len(circuit.buses), ntime=ntime)
