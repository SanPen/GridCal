...
def cim_to_mpc(cimfiles, boundary_profiles=[], log_filename=''):
    ...
    try:
        # NB: The ENTSO-E boundary files have to be listed first (before the CIM of the actual network) in order to be imported correctly !!!
        # NB: The CIM files have to be loaded in the following order: EQ, TP and then SV, in order for the RDFXMLReader to work correctly !!!
        ...
        logger.error('Some error occured during the CIM import.\nAre you sure you are importing CIMv14 ENTSO-E profile and that you are providing all the CIM files (_EQ, _TP, _SV) and boundary files (_EQ, _TP). All the files must end with the corresponding suffix and must not contain Unicode characters! \n')
    ...
    logger.info('CIM to Matpower import ENDED at %s \n' %str(datetime.now()))
...
class CIM2Matpower(object):
    ...
    *cim_file* is the path to an XML/RDF encoded CIM topology.
    ...
    def _create_branches(self, lines, p_transformers, BasePower_MVA, buses):
        ...
        Create list of :class:`Topology_NodeBreaker.Branch`, :class:`Topology_NodeBreaker.PhaseTapChanger` and :class:`Topology_NodeBreaker.RatioTapChanger` -s
        ...
        for line in lines:
            ...
            if from_bus not in buses:
                ...
                if status_connected:
                    ...
                    logging_message = 'Line %s (switched ON): the line is switched ON but terminal _T1 belongs to another island (%s) and because of that the line is not imported!' %(line.UUID, tisland_name)
                    logger.error(logging_message)
                else:
                    logger.warn('Line %s (switched off): terminal %s is connected to another island (%s) and because of that the line is not imported!' %(line.UUID, line.Terminals[0].UUID, tisland_name))
                continue

            if not to_bus:
                logging_message = 'Line %s: terminal _T1 is None and because of that the line is not imported!' % line.UUID
                logger.warn(logging_message)
            if to_bus not in buses:
                ...
                if status_connected:
                    ...
                    logging_message = 'Line %s (switched off): terminal %s is connected to another island (%s) and because of that the line is not imported!' %(line.UUID, line.Terminals[1].UUID, tisland_name)
                    logger.warn(logging_message)
                continue
            ...
            if hasattr(pt, 'TransformerWindings'):
                ...
                if len(pt.TransformerWindings) == 2:
                    ...
                    if not to_bus_primary:
                        logging_message = 'Transformer %s: the TNode of terminal of primary winding is None and because of that the transformer is not imported!' % pt.UUID
                        logger.warn(logging_message)
                    if to_bus_primary not in buses:
                        ...
                        if status_connected:
                            logging_message = 'Transformer %s (switched ON): the transformer is switched ON but terminal %s belongs to another island (%s) and because of that the line is not imported!' %(pt.UUID,pt.TransformerWindings[index_primary].Terminals[0].UUID, tisland_name)
                            logger.error(logging_message)
                            # NOTE: Maybe we should raise an error for this case
                        else:
                            logging_message = 'Transformer %s (switched off): terminal %s is connected to another island (%s) and because of that the transformer is not imported!' %(pt.UUID, pt.TransformerWindings[index_primary].Terminals[0].UUID, tisland_name)
                            logger.warn(logging_message)
                        continue

                    from_bus_secondary = self._find_tnode(pt.TransformerWindings[index_secondary].Terminals[0])
                    if not from_bus_secondary:
                        logging_message = 'Transformer %s: the TNode of terminal of secondary winding is None and because of that the transformer is not imported!' % pt.UUID
                        logger.warn(logging_message)
                    if from_bus_secondary not in buses:
                        tisland_name = pt.TransformerWindings[index_secondary].Terminals[
                            0].TopologicalNode.TopologicalIsland.UUID
                        if status_connected:
                            logging_message = 'Transformer %s (switched ON): the transformer is switched ON but terminal %s belongs to another island (%s) and because of that the line is not imported!' %(pt.UUID,pt.TransformerWindings[index_secondary].Terminals[0].UUID, tisland_name)
                            logger.error(logging_message)
                            # NOTE: Maybe we should raise an error for this case
                        else:
                            logging_message = 'Transformer %s (switched off): terminal %s is connected to another island (%s) and because of that the transformer is not imported!' %(pt.UUID, pt.TransformerWindings[index_secondary].Terminals[0].UUID, tisland_name)
                            logger.warn(logging_message)
                        continue
                    ...
                    if pt.TransformerWindings[index_secondary].r or pt.TransformerWindings[index_secondary].x or \
                        ...
                        logging_message = 'The two-windings transformer %s has impedance and/or admittance on its secondary winding!' % pt.UUID
                        logger.warn(logging_message)
                    ...
                    if pt.TransformerWindings[index_primary].PhaseTapChanger and pt.TransformerWindings[
                        index_secondary].PhaseTapChanger:
                        logging_message = 'The two-windings transformer %s has PhaseTapChanger on its primary winding!' % pt.UUID
                        logger.warn(logging_message)
                    if ptc:
                        ...
                        if ptc.voltageStepIncrementOutOfPhase and ptc.voltageStepIncrementOutOfPhase != 0:
                            ...
                            logging_message = 'PhaseTapChanger %s of power transformer %s does not have a valid value for voltageStepIncrementOutOfPhase or stepVoltageIncrement attribute, so by default it is set to 1!' %(ptc.UUID, pt.UUID)
                            logger.warn(logging_message)
                            du = 1 / 100
                        ...
                        if ptc.windingConnectionAngle:
                            ...
                            logging_message = 'PhaseTapChanger %s of power transformer %s does not have windingConnectionAngle attribute, so by default it is set to 1!' %(ptc.UUID, pt.UUID)
                            logger.warn(logging_message)
                            theta_rad = math.pi / 2

                        if ptc.xStepMin < 0 or ptc.xStepMax <= 0 or ptc.xStepMin > ptc.xStepMax:
                            xStepRangeIsInconsistent = True
                            logging_message = 'xStepMin and xStepMax of PhaseTapChanger %s of PowerTransformer %s are inconsistents!' %(ptc.UUID, pt.UUID)
                            logger.warn(logging_message)
                        else:
                    ...
                    if pt.TransformerWindings[index_primary].RatioTapChanger:
                        if pt.TransformerWindings[index_secondary].RatioTapChanger:
                            logging_message = 'The two-windings transformer %s has RatioTapChnager on both windings!' % pt.UUID
                            logger.warn(logging_message)
                        else:
                            rtc = pt.TransformerWindings[index_secondary].RatioTapChanger
                            if rtc:
            ...
            if rtc:  # RatioTapChanger
                if rtc.RegulatingControl:
                    self.node_breaker_data.ratiotapchangers.append(
                        Topology_NodeBreaker.RatioTapChanger(branch_id=self.node_breaker_data.branches[-1].id, id_cim=rtc.UUID, lowStep=rtc.lowStep,
                                            highStep=rtc.highStep, neutralU_kV=rtc.neutralU,
                                            stepVoltageIncrement_kV=rtc.stepVoltageIncrement,
                                            continuousPosition=rtc.SvTapStep.continuousPosition,
                                            hasRegulatingControl=True, RC_discrete=rtc.RegulatingControl.discrete,
    ...
    def _create_buses_and_gens(self, tnodes):
        ...
        for voltage_level, tnodes_set in self.voltage_levels.items():  # for each VoltageLevel
            for tnode in tnodes_set:
                self.cim_nodes[tnode] = Topology_NodeBreaker.Node(id_cim=tnode.UUID, name=tnode.name, desc=tnode.description)
            ...
            for equip in voltage_level.Equipments:
                if isinstance(equip, self._get_cls('Switch')):
                    ...
                    if bool(tnode_from.TopologicalIsland is not self.biggest_tnode_island) ^ bool(tnode_to.TopologicalIsland is not self.biggest_tnode_island):
                        ...
                        logging_message = 'Switch "%s" is switched ON and is connected between two different TopologicalIslands (%s and %s) and thus will not be imported !!!' % (
                            equip.UUID, tnode_from.TopologicalIsland.UUID, tnode_to.TopologicalIsland.UUID)
                        logger.error(logging_message)
            ...
            for equip in prim_equip:
                ...
                if equip_cls == 'SynchronousMachine':
                    ...
                    if equip.minQ:
                        ...
                        if equip.InitialReactiveCapabilityCurve:
                            ...
                            # ToDo: Does this needs to be recalculated? As far as I understand it is given for a specific P
                        else:
                            Qmin_MVAr = 0
                    ...
                    if equip.operatingMode == 'generator':
                        bus_pv = Topology_BusBranch.bus_type.PV # ToDo: This logic for defining the type of bus might not be the best... Instead maybe look at VoltageControl?
                ######################################
                ...
                ######################################
                    ...
                    self.node_breaker_data.shunts.append(
                        Topology_NodeBreaker.Shunt(node_id=self.cim_nodes[tnode].id, id_cim=equip.UUID, name=equip.name,
                                    status=status,
    ...
    def _iterate_prim_eq(self, cim_objects):
        ...
        for mrid, obj in cim_objects.items():
            if isinstance(obj, self._prim_onet) or isinstance(obj, self._prim_twot):
                ...
                self.cim_areas[obj] = Topology_NodeBreaker.Area(id_cim=obj.UUID, name=obj.name,
                                               netInterchange=obj.netInterchange, pTolerance=obj.pTolerance)
            elif isinstance(obj, self._get_cls('SubGeographicalRegion')):
                self.cim_zones[obj] = Topology_NodeBreaker.Zone(id_cim=obj.UUID, name=obj.name)
