...
\begin{equation}
V_{i+1} = V_i + J^{-1}(S_0 - S_{calc})
\end{equation}

Where:
\begin{itemize}
	\item $V_i$: Voltage vector at the iteration $i$  (current voltage) 
	\item $V_{i+1}$: Voltage vector at the iteration $i+1$  (new voltage)
	\item $J$: Jacobian matrix.
	\item $S_0$: Specified power vector.
	\item $S_{calc}$: Calculated power vector using $V_i$.
\end{itemize}

The version implemented in GridCal is the following:

\begin{equation}
V_{i+1} = V_i + \mu J^{-1}(S_0 - S_{calc})
\end{equation}

Where $\mu$ is the Iwamoto acceleration parameter. In 1982 S.Iwamoto and Y.Tamura present a method \cite{iwamoto1981load} where the Jacobian matrix $J$ is only computed at first, and an iteration control parameter $\mu$ is computed on every iteration. In GridCal I compute $J$ and $\mu$ on every iteration getting a more robust method on the expense of a greater computational effort.
...
$$a = S_0 - S_{calc}$$
$$b = J \times dx$$
$$c = \frac{1}{2} dx \cdot (J' \times dx)$$

$$g_0 = -a \cdot b$$
$$g_1 = b \cdot b + 2  a \cdot c$$
$$g_2 = -3  b \cdot c$$
$$g_3 = 2  c \cdot c$$
...
There will be three solutions to the polynomial $G(x)$. Only the last solution will be real, and therefor it is the only valid value for $\mu$.
The polynomial can be solved numerically using $1$ as the seed.

The matrix $J'$ is the Jacobian matrix computed using the voltage derivative (or voltage increment $dV= V_{i} - V_{i-1}$).  $J' = Jacobian(Y, dV)$
...
The equation \ref{eq:McLaurinV} provides us an expression to obtain the voltage out of the coefficients, knowing that $\alpha$ should be $1$ to get the final voltage results. So, why do we need any further operation?, and what is this Padè thing?
...
The Padè approximation is a rational approximation of a function (the voltage series in our case). The approximation is valid over a small domain of the function, in our case the domain is $\alpha=[0,1]$. The method requires the function to be continuous and differentiable for $\alpha=0$. Hence the Cauchy-Riemann condition (So, yes our function meets this condition).
...
\begin{equation}
Voltage\_value\_approximation = \frac{P_N(\alpha)}{Q_M(\alpha)} \quad \forall \quad 0 \leq \alpha \leq 1
\label{eq:pade_apprx}
\end{equation}
...
The problem now boils down to find the $q_i$ and $p_i$ coefficients by solving two systems of equations. The first one to find $q_i$ which does not depend on $p_i$, and the second one to get $p_i$ which does depend on $q_i$.
...
Once the coefficients are there, it is only a matter of evaluating the equation \ref{eq:pade_apprx} for $\alpha=1$.
