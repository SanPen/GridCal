...
It assumes that in any branch the reactive part is much larger than the resistive part, hence the resistive part is neglected, and that all the voltages modules are the nominal per unit values. This is, $|v|=1$ for load nodes and $|v|=v_{set}$ for the generator nodes, where $v_{set}$ is the generator set point value.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.6\linewidth]{Matrix_reduction.eps}
