...
\input{structureB} % Insert the commands.tex file which contains the majority of the structure behind the template
...
\node[font=\normalfont\fontsize{70}{70}\sffamily\selectfont, color=white] at ([yshift=17cm]current page.south) {GridCal};

\node[font=\normalfont\fontsize{35}{35}\sffamily\selectfont \Huge, color=white] at ([yshift=15cm]current page.south) {Santiago Peñate Vera};
\end{tikzpicture}
...
\begin{itemize}
	...
	\item $controlled\_generators$: List of Generator objects.
...
\begin{itemize}
	...
	\item $Vset$: Voltage set point (p.u.)
	\item $Vset\_prof$ Pandas DataFrame with the voltage set point profile (p.u.)
	\item $name$: Name of the Generator.
\end{itemize}
